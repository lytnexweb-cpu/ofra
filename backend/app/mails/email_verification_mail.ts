...
export default class EmailVerificationMail extends BaseMail {
  ...
  constructor(opts: {
    ...
    const t = getTranslation('emailVerification', this.lang)
  ...
  private buildHtml(): string {
    ...
    const body = `
      ...
      <div class="text-center">
        <a href="${this.verifyUrl}" class="cta-button">${t.cta}</a>
      </div>
      ...
      <p class="text-small text-muted mt-4">
        ${t.linkFallback}<br>
        <a href="${this.verifyUrl}" style="color: ${OFRA_COLORS.accent}; word-break: break-all;">${this.verifyUrl}</a>
      </p>
    `
